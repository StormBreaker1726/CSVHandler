...
bool write()
{
    CSVWriter csv_test_writer("csv_test_writer.csv", 3, false);
    ...
    for (size_t i = 0; i < 5; i++)
    {
        ...
        row.push_back(std::to_string(6*(i+b)));
        csv_test_writer.write_in_file(row);    
    }
...
bool read_row(int row_to_read)
{
    CSVReader csv_test_reader("csv_test_writer.csv", 3);

    std::vector<std::string> row = csv_test_reader.read_row(row_to_read);
